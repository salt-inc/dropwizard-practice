...
function search(pageCount) {
	...
	// 検索実行メソッドを呼び出す
	searchRun(url, "GET", occupationTypeId, industryTypeId, freeWord, count);
}

// 検索実行メソッド
function searchRun(Url, Method, oTypeId, iTypeId, fWord, pageCount) {
	...
	$.ajax({
        ...
        // 検索ボタン押下時のみ最大ページ数を更新する
        if (pageCount == 0) {
        	count += 1;
        	maxPageCount = Math.floor((data.length + 9) / 10)
        	...
        	$("#maxPageCount").empty();
        	$("#maxPageCount").append(maxPageCount);
        }
        ...
        if (count != 1) {
        	$("#pageCount").before("<input type=button id=backButton value=前へ onClick=search(-1)>");
        }

        if (count != maxPageCount) {
        	$("#maxPageCount").after("<input type=button id=nextButton value=次へ onClick=search(1)>");
        }
